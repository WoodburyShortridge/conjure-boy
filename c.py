...
def run_game(boy):
    ...
    while not boy.tick():
        ...
        if isinstance(frame1, np.ndarray):
            ...
            cachedContours = cachedContours.append(newContours)
            draw_contours(cachedContours, lightFrame)
            contours = cachedContours

        darkFrame = filter_light_dark(lightFrame, contours)

        frame1 = frame2
        cv2.imshow('contours', darkFrame)

        transformData = transform_translation(frame2)
